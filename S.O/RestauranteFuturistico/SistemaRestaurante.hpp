...
private:
    ...
    std::list<Chef*> disponbiveis;
    std::map<int, Chef*> mesaParaChef;
    std::queue<std::pair<int, std::string>> mesaPedidos;
    int totalMesas;
...
public:
    ...
    void iniciarAtendimento(int qtdMesas);

    void encerrarMesa (int numMesa, std:: string);

    void consumirPedidoFila ();
